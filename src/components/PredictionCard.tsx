...
export default function PredictionCard({ prediction, currentUserId, onOutcomeUpdate, onHide }: PredictionCardProps) {
  ...
  return (
    ...
    >
      ...
      {/* 5. ACTIONS ROW - Purple/Cyan theme buttons with prominent glow */}
      <div className={`grid gap-2 items-stretch mt-auto ${canResolve ? 'grid-cols-[1.5fr_1fr_auto]' : 'grid-cols-[1fr_auto]'}`}>
        ...
        <button
          ...
          className="px-4 py-2.5 text-sm font-semibold text-cyan-200 bg-cyan-500/10 rounded-lg transition-all border border-cyan-500/40 hover:border-cyan-400/60 hover:bg-cyan-500/20 hover:shadow-[0_0_20px_rgba(6,182,212,0.3)] flex items-center justify-center gap-2"
        >
          <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        {canResolve && (
          <button
            ...
            className="px-3 py-2.5 text-sm font-semibold rounded-lg transition-all border-2 text-purple-200 bg-purple-500/20 hover:bg-purple-500/30 border-purple-500/50 hover:border-purple-400/70 hover:shadow-[0_0_20px_rgba(168,85,247,0.4)] flex items-center justify-center gap-1.5"
          >
            Resolve
          </button>
        )}
        ...
        <button
          ...
          className="w-11 h-full text-sm font-semibold text-purple-200 bg-purple-500/10 rounded-lg transition-all border border-purple-500/40 hover:border-purple-400/60 flex items-center justify-center hover:bg-purple-500/20 hover:shadow-[0_0_20px_rgba(168,85,247,0.3)]"
        >
          {linkCopied ? (
