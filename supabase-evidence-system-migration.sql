...
-- Add constraint for evidence_summary length
ALTER TABLE predictions
ADD CONSTRAINT IF NOT EXISTS evidence_summary_length
...
-- Policy: Users cannot update or delete evidence (immutable)
-- (No policies = no one can update/delete except via service role)
...
SELECT
  ...
  COUNT(DISTINCT id) as total_evidence_items,
  COUNT(DISTINCT user_id) as users_with_stats
FROM predictions
LEFT JOIN evidence_items ON predictions.id = evidence_items.prediction_id
LEFT JOIN user_stats ON predictions.user_id::UUID = user_stats.user_id
WHERE predictions.resolved_at IS NOT NULL;
