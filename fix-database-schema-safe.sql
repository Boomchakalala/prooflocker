...
-- 100% SAFE - ADDITIVE ONLY, won't break existing data
...
DO $$
BEGIN
  ALTER TABLE predictions ALTER COLUMN author_number DROP NOT NULL;
EXCEPTION
  WHEN others THEN NULL; -- Ignore if already nullable
END $$;
...
-- Add constraints (wrapped in DO blocks to ignore if they already exist)
DO $$
BEGIN
  ALTER TABLE predictions
  ADD CONSTRAINT resolution_note_length CHECK (resolution_note IS NULL OR LENGTH(resolution_note) <= 280);
EXCEPTION
...
BEGIN
  ALTER TABLE predictions
  ADD CONSTRAINT resolution_url_format CHECK (resolution_url IS NULL OR resolution_url ~ '^https?://.+');
EXCEPTION
  WHEN duplicate_object THEN NULL; -- Constraint already exists
END $$;
...
BEGIN
  ALTER TABLE predictions
  ADD CONSTRAINT predictions_outcome_check CHECK (outcome IN ('pending', 'correct', 'incorrect', 'invalid'));
EXCEPTION
  WHEN duplicate_object THEN NULL; -- Constraint already exists
END $$;
...
BEGIN
  ALTER TABLE predictions
  ADD CONSTRAINT predictions_category_check CHECK (category IN ('Crypto', 'Politics', 'Markets', 'Tech', 'Sports', 'Culture', 'Personal', 'Other'));
EXCEPTION
  WHEN duplicate_object THEN NULL; -- Constraint already exists
END $$;
...
BEGIN
  ALTER TABLE predictions
  ADD CONSTRAINT predictions_moderation_status_check CHECK (moderation_status IN ('active', 'hidden'));
EXCEPTION
  WHEN duplicate_object THEN NULL; -- Constraint already exists
END $$;

-- Update existing records to have default values (SAFE - only touches NULL values)
